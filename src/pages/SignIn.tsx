...
export default function SignIn() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        // Successfully signed in; notify shell to fetch profile
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-y-auto p-3 sm:p-4" style={{ backgroundColor: '#0e0e14' }}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-pink-500 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-2000"></div>
        <div className="absolute top-1/2 left-1/2 w-96 h-96 bg-indigo-500 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-blob animation-delay-4000"></div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md my-4">
        ...
        >
          ...
          <div className="text-center mb-4">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-gradient-to-br from-purple-500 to-pink-500 mb-3 shadow-lg">
              <Sparkles className="w-6 h-6 text-white" />
            </div>
            <h1 className="text-2xl font-bold text-white mb-1">Welcome Back</h1>
            <p className="text-sm text-purple-200">Sign in to continue to LawPal</p>
          </div>

          {/* Error message */}
          {error && (
            <div className="mb-3 p-2.5 rounded-xl bg-red-500/20 border border-red-500/30 backdrop-blur-sm">
          ...
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-3 text-gray-400 text-xs" style={{ backgroundColor: '#0e0e14' }}>Or continue with</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <button
              ...
              className="py-2.5 rounded-xl bg-white/10 border border-white/20 text-white text-sm font-medium hover:bg-white/20 transition-all backdrop-blur-sm flex items-center justify-center gap-1.5"
            >
          ...
          <p className="mt-4 text-center text-xs text-purple-200">
            ...
            <Link
              to="/signup"
              className="font-semibold text-white hover:text-purple-100 transition-colors"
            >
              Sign up
            </Link>
          </p>
